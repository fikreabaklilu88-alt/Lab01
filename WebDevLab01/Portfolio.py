...
def about_me_section():
    st.header("🚀About Me")                       #enlarges the words on the website
    st.image(info.profile_picture, width = 200) #makes it so the picture is there
    st.write(info.about_me)         #basically a print function
    st.write("---")                 #used to make a blank line between the things
...
#Sidebar Links
def links_section():
    st.sidebar.header("Links")      #Gives the page a sidebar, and then a header
    st.sidebar.text("Connect with me on Linkedin")          #gives you text in everything
...
def education_section(education_data, course_data):
    st.header("📖Education")
    st.subheader(f"**{education_data['Institution']}**")    #The ** make words bold
